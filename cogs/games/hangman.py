...
class HangmanGameAll(HangmanGame):
    ...
    def new_input(self, guess, player_id):
        ...
        if not correct:
            self.player_cds[player_id] = datetime.datetime.now() + datetime.timedelta(seconds=2.5)
        return correct, won
...
def hangman_embed(unveiled, guessed, player_cds):
    ...
    embed = discord.Embed(title='Hangman - Free For All', description=description, color=discord.Color(0xCCCC00))
    if guessed:
...
def hangman_embed_winner(secret, guessed, player, time):
    ...
    embed = discord.Embed(title='Hangman - Free For All', description=description, color=discord.Color(0x008000))
...
def hangman_embed_timeout(secret):
    ...
    embed = discord.Embed(title='Hangman - Free For All', description=description, color=discord.Color(0x8B0000))
    ...
    async def hangall(self, ctx):
        ...
        while not game_done:
            ...
            if won:
                ...
                embed = hangman_embed_winner(unveiled, guessed, msg.author, datetime.datetime.now() - start_time)
                await game_embed.edit(embed=embed)
