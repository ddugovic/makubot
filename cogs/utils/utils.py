...
class Utils(commands.Cog):
    ...
    async def on_ready(self):
        ...
        await self.bot_log.send(
            f"Starting up.. Running on {len(self.bot.guilds)} servers. Starttime: {self.bot.starttime}."
        )
    ...
    async def ping(self, ctx):
        ...
        elapsed_ms = (
            (diff.days * 86400000) + (diff.seconds * 1000) + (diff.microseconds / 1000)
        )
    ...
    async def servertime(self, ctx):
        ...
        await ctx.channel.send(
            f"Current Servertime: `{datetime.now().replace(microsecond=0)}`."
        )
    ...
    async def serverinfo(self, ctx):
        ...
        embed = discord.Embed(
            title=name, description=f"Server Information", color=discord.Color(0xFF8EED)
        )
        ...
        embed.add_field(
            name="Users",
            value=f"{len(members)} ({len([member for member in members if member.status is not discord.Status.offline])} online)",
        )
    ...
    async def userinfo(self, ctx, *args):
        ...
        embed = discord.Embed(
            title=str(user), description=f"User Information", color=user.color
        )
        ...
        embed.add_field(
            name="Created At", value=f"{user.created_at.replace(microsecond=0)}"
        )
        embed.add_field(
            name="Joined At", value=f"{user.joined_at.replace(microsecond=0)}"
        )
    ...
    async def botinfo(self, ctx):
        ...
        embed = discord.Embed(
            title="Maki",
            description=f"[*your digital assistant*](https://discordbots.org/bot/431485759304892416)",
            color=discord.Color(0x963826),
        )
        embed.set_thumbnail(url=user.avatar_url)
        ...
        embed.add_field(
            name="Total Channels", value=len(set(self.bot.get_all_channels()))
        )
        await ctx.channel.send(embed=embed)
