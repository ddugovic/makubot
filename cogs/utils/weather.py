...
class Weather(commands.Cog):
    ...
    async def weather(self, ctx, *location: str):
        ...
        try:
            ...
            await ctx.channel.send(
                f"⚠️ Requested location `{location}` not found. Please try a different location."
            )
        except Exception as ex:
    ...
    def weather_build_embed(self, location):
        ...
        temp_max_str = (
            f"{temp_c.get('temp_max', None)}°C / {temp_f.get('temp_max', None)}°F"
        )
        temp_min_str = (
            f"{temp_c.get('temp_min', None)}°C / {temp_f.get('temp_min', None)}°F"
        )
